const links = [
  ...
  {name:"Gonzalo Fernandez",facebook:"https://www.facebook.com/Dsnfl/mentions/"},
  ...
  {name:"Smolen & Roytman, PLLC",facebook:"https://www.facebook.com/SmolenamdRoytman"},
...
async function check(name, url) {
  try {
    ...
    const bad =
      ...
      body.includes('"errorCode":"PageNotFound"') ||
      body.includes("content_not_found") ||
      body.includes("This content isn") ||
      body.includes("page you requested cannot be found") ||
      body.includes("Sorry, this page") ||
      res.status === 404
    return { name, url, status: res.status, finalUrl, bad }
  } catch(e) {
    return { name, url, status: "ERR", finalUrl: "", bad: true, err: e.message }
  }
}
...
async function run() {
  ...
  for (let i = 0; i < links.length; i += 10) {
    ...
    const batchResults = await Promise.all(batch.map(l => check(l.name, l.facebook)))
    results.push(...batchResults)
    process.stdout.write(".")
  }
  console.log()
  const bad = results.filter(r => r.bad)
  const ok = results.filter(r => !r.bad)
  console.log("\nSUSPECT (" + bad.length + "):")
  bad.forEach(r => console.log("  " + r.name + "\n    url: " + r.url + "\n    final: " + r.finalUrl + " [" + r.status + "]" + (r.err ? "\n    err: " + r.err : "")))
  console.log("\nOK (" + ok.length + "): " + ok.map(r => r.name).join(", "))
}
run()
